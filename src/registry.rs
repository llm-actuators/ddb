...
pub struct Device {
    ...
    pub enrolled: String,
    /// Last time ddb successfully contacted this device. Bumped by the
    /// claim wrapper on every successful claim acquire. Used by the
    /// 30-day auto-prune (operator directive 2026-06-23 msg `4ovb3n` +
    /// `x58xvk`: "operator prunes nothing"). Optional so existing
    /// enrollment files load without migration.
