...
/// True for subcommands that physically touch the device. These acquire
/// the claim before dispatch; everything else (registry queries, config,
/// daemon control, doctor checks, completion script) skips the claim.
fn needs_device_claim(cmd: &Command) -> bool {
    !matches!(
        cmd,
        Command::Devices(_)
            | Command::Daemon(_)
...
pub fn run(cli: Cli) -> Result<(), String> {
    ...
    // Device-bound mutating commands acquire a claim before dispatch.
    // See SPEC-device-claim.md. Registry/config/help-style commands skip
    // the check — they don't touch the physical device.
    if needs_device_claim(&cli.command) {
        crate::claim_wrapper::check_or_acquire(dev)?;
    }
    match cli.command {
