...
struct StepRaw {
    ...
    // TD-160 — ordered list of targets for assert_order.
    #[serde(default)]
    targets: Option<Vec<Target>>,
    // TD-161 — assert_responsive ceiling (ms).
    #[serde(default)]
...
struct AssertStep {
    ...
    hint: Option<String>,
    enabled: Option<bool>,
    // TD-160 — ordered targets list for assert_order.
    targets: Option<Vec<Target>>,
    // TD-161 — assert_responsive ceiling (ms).
    max_idle_ms: Option<u64>,
}
...
pub fn run(dev_name: Option<&str>, args: TestArgs) -> Result<(), String> {
    ...
    for (tc_index, spec_path) in specs.iter().enumerate() {
        ...
        //
        // Per-TC ts_slug is computed ONCE here and reused for both the
        // pre-run capture and the post-run emit_o1_bundle call. Previously
        // each path computed its own now_timestamp_slug(); if the TC took
        // >1s (always), the slugs differed → start.png landed under run-id
        // A, end.png + manifest under run-id B, manifest.screenshots[]
        // scan in B never saw A's start.png.
        ...
        let early_bundle_dir: Option<std::path::PathBuf> = results_dir_early.as_ref().map(|d| {
            let run_id = std::env::var("DDB_RUN_ID")
                .unwrap_or_else(|_| format!("{}-{:x}", ts_slug, std::process::id()));
        ...
        if let Some(ref dir) = results_dir {
            ...
            // ts_slug already computed once at outer-loop start so pre-run
            // capture + post-run emit_o1_bundle share the same run-id dir.
...
fn emit_o1_bundle(
    ...
    // TD-145: fall back to DDB_TEST_PACKAGE env when spec.precondition.package
    // is None — apk fingerprint doesn't depend on the agent, so even a TC that
    // failed pre-agent-up should still record what was on the device.
    let pkg_env: String;
    let pkg_resolved = match package {
        Some(p) => Some(p),
        None => {
            pkg_env = std::env::var("DDB_TEST_PACKAGE").unwrap_or_default();
            if pkg_env.is_empty() { None } else { Some(pkg_env.as_str()) }
        }
    };
    if let (Some(d), Some(pkg)) = (dev, pkg_resolved) {
        if let Ok(out) = std::process::Command::new("adb")
    ...
    let build_fingerprint = serde_json::json!({
        ...
        "agent_sha": agent_sha,
        "package": pkg_resolved.unwrap_or(""),
...
fn now_iso() -> String {
    // TD-144: real ISO-8601 from chrono (was previously hand-rolled with
    // a broken month/day calculation — every timestamp landed on Jan 1).
...
fn execute_assert(dev: Option<&Device>, assert: &AssertStep, timeout: u64, ctx: &RunContext, runner: &StepRunner) -> Result<String, String> {
    match assert.assert.as_str() {
        ...
        "element_state" => {
            ...
            // visible label (production app catalogue convention)
        ...
        // TD-160 — assert_order: ordered-existence over /semantic.
        // YAML:
        //   - assert: element_order
        //     targets:
        //       - {content_fuzzy: "Facts"}
        //       - {content_fuzzy: "Regulations"}
        "element_order" => {
            let targets = assert.targets.as_ref()
                .ok_or_else(|| "assert element_order: targets list missing".to_string())?;
            if targets.is_empty() {
                return Err("assert element_order: targets list empty".into());
            }
            let elements = get_semantic_elements(dev)?;
            let mut cursor: i64 = -1;
            for (i, t) in targets.iter().enumerate() {
                let desc = t.content_fuzzy.as_deref()
                    .or(t.id.as_deref())
                    .unwrap_or("(unnamed)");
                let mut found: Option<usize> = None;
                for (idx, el) in elements.iter().enumerate() {
                    if (idx as i64) <= cursor { continue; }
                    let m = match (t.content_fuzzy.as_deref(), t.id.as_deref()) {
                        (Some(f), _) => el.to_lowercase().contains(&f.to_lowercase()),
                        (None, Some(id)) => {
                            el.contains(&format!("platform_id: \"{id}\""))
                            || el.contains(&format!("id: \"{id}\""))
                        }
                        _ => false,
                    };
                    if m { found = Some(idx); break; }
                }
                let pos = found.ok_or_else(|| format!(
                    "element_order: target #{} '{}' not found after position {} ({} elements)",
                    i + 1, desc, cursor, elements.len()
                ))?;
                cursor = pos as i64;
            }
            Ok(format!("order verified across {} targets", targets.len()))
        }
        // TD-161 — assert_responsive: proxy for main-thread responsiveness.
        // Hits agent /health with a ms budget. If round-trip exceeds
        // max_idle_ms (default 500ms), the UI thread is assumed pegged.
        "responsive" => {
            let budget_ms = assert.max_idle_ms.unwrap_or(500);
            let agent_url = agent_base_url();
            let url = format!("{agent_url}/health");
            let timeout_sec = (budget_ms as f64 / 1000.0) + 0.5;
            let start = std::time::Instant::now();
            let out = std::process::Command::new("curl")
                .args(["-s", "-o", "/dev/null", "-w", "%{http_code}",
                       "--max-time", &format!("{timeout_sec:.2}"), &url])
                .output();
            let elapsed_ms = start.elapsed().as_millis() as u64;
            let ok = out.as_ref().map(|o| o.status.success()).unwrap_or(false);
            if !ok {
                return Err(format!(
                    "responsive: agent /health unreachable after {elapsed_ms}ms (budget {budget_ms}ms) — main thread likely frozen"
                ));
            }
            if elapsed_ms > budget_ms {
                return Err(format!(
                    "responsive: /health took {elapsed_ms}ms > budget {budget_ms}ms"
                ));
            }
            Ok(format!("responsive: /health {elapsed_ms}ms ≤ {budget_ms}ms"))
        }
...
"#;
    ...
    fn test_fixture_interpolation_integer_fields() {
        ...
        map.insert("{{fixtures.test_site.id}}".to_string(), "10001".to_string());
...
"#;
        ...
        assert!(interpolated.contains("10001"), "site id should be interpolated");
    ...
    fn test_fixture_interpolation_string_fields() {
        ...
        map.insert("{{fixtures.alex.name}}".to_string(), "Test User".to_string());
...
"#;
        ...
        assert_eq!(map.get("{{fixtures.alex.id}}"), Some(&"14".to_string()));
    }

    #[test]
    fn test_token_jaccard() {
        assert!(token_jaccard("activity feed", "activity feed") >= 0.99);
        assert!(token_jaccard("activity", "activity feed") > 0.3);
        assert!(token_jaccard("xyz", "activity feed") < 0.1);
    }
    ...
    fn test_jaccard_threshold_boundary() {
        // "activity feed" vs "activity feed updates" — 2/3 overlap
        let score = token_jaccard("activity feed", "activity feed updates");
        ...
        let exact = token_jaccard("activity feed", "activity feed updates");
        // 2 shared (activity, feed) / 3 union (activity, feed, updates) = 0.667
