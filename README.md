...
| `semantic-schema/` | Shared schema crate | `git submodule` → `maintainer/semantic-schema` |
...
ddb app list myapp                 # filter by keyword
...
## Per-TC artifact bundles (Epic O, Wave 29)

`ddb test` writes a permanent on-disk bundle per TC under `<tests-parent>/results/<run-id>/<tc-id>/`:

```
manifest.json    # canonical schema per ADR-016: tc_id, tc_name, platform, status, started, finished, device, run_id, ts_slug, spec_path, steps_run, steps_total, agent_sha, screenshots, walker_dumps, build_fingerprint, calibration, failure_diagnostic
tc.jsonl         # per-step JSONL trace (action/assert, target, outcome, ms)
tc.log           # raw runner stdout/stderr
start.png        # screenshot pre-TC (adb exec-out screencap)
end.png          # screenshot post-TC
fail-step-N.png  # FAIL only: screenshot at the failing step
semantic-start.yaml + semantic-end.yaml  # walker dumps pre/post
api-log.jsonl    # mocks served + intercepts (post-run drain of agent /debug-log)
logcat.txt       # FAIL only: logcat-since-TC-start (adb logcat -d -T <cursor>)
error.json       # FAIL only: failed step + expected-vs-actual + walker state, mirrors manifest.failure_diagnostic 1:1
```

Run-id format: ISO-UTC + 6-char hex, propagated via `DDB_RUN_ID` env var (sweep callers set it; standalone runs generate it). Retention controlled by `DDB_RETAIN_RUNS` (default 10) + pinned-runs exempt.

Bundles are the source of truth for failure triage (per ADR-015) — stdout, scrollback, and operator memory are not. Inspect with `tctl view --results <dir>` (single-page HTML drill-down) or gate commits with `tctl bundle-gate --results <dir>` (exits 1 if newest bundle has any non-PASS TC).
...
- [idb](https://github.com/maintainer/idb) — iOS Device Bridge (Swift)
- [wdb](https://github.com/maintainer/wdb) — Web Debug Bridge (Go)
